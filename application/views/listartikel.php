	...
	<h1>List Artikel</h1>
		...
		<table style="width: 100%;">
				<thead>
					...
					<td>Id Artikel</td>
					<td>Judul Artikel</td>
					<td>Pangantar Artikel</td>
					<td>Deskripsi Artikel</td>
					<td>Gambar</td>
				</thead>
				<?php echo form_open(site_url('admin/c_admin/deleteartikel')); ?>
				<tbody>
				<?php foreach($dataArtikel as $x) { ?>
					<tr>
					<td><input type="checkbox" name="artikel[]" value="<?php echo $x['id_artikel']?>"></td>
					<td><?= $x['id_artikel'] ?></td>
					<td><?= $x['judul_artikel'] ?></td>
					<td><?= $x['pengantar_artikel'] ?></td>
					<td><?= $x['deskripsi_artikel'] ?></td>
					<td style="text-align: center;" style="width: 50%;"><img src="<?php echo base_url($x['patch_artikel']);?>" style="height: 100px;"></td>
					</tr>
				<?php } ?>
				</tbody>
			</table><br><br>
